...
export default function CreateOrderScreen() {
  ...
  const items: OrderItem[] = useMemo(() => {
    return LAUNDRY_SERVICES.map((service) => {
  ...
  const totalAmount = servicesTotal + urgentFee;
  const finalDeliveryAddress = sameAsPickup ? pickupAddress : deliveryAddress;

  const useCurrentLocation = async () => {
    setError("");
    if (Platform.OS === "web") {
      setPickupAddress("Current location selected from browser");
      return;
    }
    const { status } = await Location.requestForegroundPermissionsAsync();
    if (status !== "granted") {
      setError("Location permission was not granted. Please type your pickup address.");
      return;
    }
    const location = await Location.getCurrentPositionAsync({});
    setPickupAddress(`Current location: ${location.coords.latitude.toFixed(5)}, ${location.coords.longitude.toFixed(5)}`);
  };
  ...
  const submit = async () => {
    ...
    const result = await createOrder({
      ...
      items: urgent
        ? [
            ...items,
            { id: "urgent-fee", serviceName: "Urgent Handling", quantity: 1, pricePerUnit: urgentFee, total: urgentFee },
          ]
        : items,
      totalAmount,
  ...
  const updateQuantity = (serviceId: string, change: number) => {
    setQuantities((current) => {
      const next = Math.max(0, (current[serviceId] ?? 0) + change);
    ...
    >
        ...
        {STEPS.map((label, index) => (
          <View key={label} style={styles.stepItem}>
            <View style={[styles.stepDot, { backgroundColor: index <= step ? colors.primary : colors.muted }]}>
              <Text style={[styles.stepNumber, { color: index <= step ? colors.primaryForeground : colors.mutedForeground }]}>{index + 1}</Text>
            </View>
      ...
      {step === 0 && (
        <View style={styles.card}>
          ...
          <Text style={[styles.subtitle, { color: colors.mutedForeground }]}>Where should the dispatcher collect the laundry?</Text>
          <TextInput
            value={pickupAddress}
          ...
          />
          <Pressable onPress={useCurrentLocation} style={[styles.secondaryButton, { borderColor: colors.primary, borderRadius: colors.radius }]}>
      ...
      {step === 2 && (
        <View style={styles.card}>
          ...
          {LAUNDRY_SERVICES.filter((service) => service.id !== "express-fee").map((service) => (
            <View key={service.id} style={[styles.serviceRow, { borderBottomColor: colors.border }]}>
              <View style={styles.serviceInfo}>
                ...
                <Text style={[styles.serviceDesc, { color: colors.mutedForeground }]}>₦{service.pricePerUnit.toLocaleString()} / {service.unit}</Text>
              </View>
              <View style={styles.qtyRow}>
                <Pressable onPress={() => updateQuantity(service.id, -1)} style={[styles.qtyButton, { backgroundColor: colors.muted }]}>
                  <Feather name="minus" size={14} color={colors.foreground} />
                </Pressable>
                <Text style={[styles.qtyText, { color: colors.foreground }]}>{quantities[service.id] ?? 0}</Text>
                <Pressable onPress={() => updateQuantity(service.id, 1)} style={[styles.qtyButton, { backgroundColor: colors.primary }]}>
        ...
        <View style={styles.card}>
          ...
          {items.map((item) => (
            <View key={item.id} style={styles.summaryRow}>
              <Text style={[styles.summaryText, { color: colors.foreground }]}>{item.quantity}x {item.serviceName}</Text>
          ...
          <View style={styles.switchRow}>
            <View style={styles.serviceInfo}>
              ...
              <Text style={[styles.serviceDesc, { color: colors.mutedForeground }]}>Adds ₦2,000 priority handling when available</Text>
            </View>
            <Switch value={urgent} onValueChange={setUrgent} trackColor={{ false: colors.muted, true: colors.accent }} thumbColor={urgent ? colors.primary : colors.card} />
          </View>
          {urgent && (
            <View style={styles.summaryRow}>
              <Text style={[styles.summaryText, { color: colors.foreground }]}>Urgent Handling</Text>
              <Text style={[styles.summaryText, { color: colors.foreground }]}>₦{urgentFee.toLocaleString()}</Text>
            </View>
          )}
          <TextInput
            value={specialRequests}
            onChangeText={setSpecialRequests}
            placeholder="Special requests or fabric notes"
            placeholderTextColor={colors.mutedForeground}
            multiline
      ...
      <View style={styles.navRow}>
        {step > 0 && (
          <Pressable onPress={() => setStep(step - 1)} style={[styles.backButton, { borderColor: colors.border, borderRadius: colors.radius }]}>
        ...
        <Pressable
          ...
          style={[styles.primaryButton, { backgroundColor: colors.primary, borderRadius: colors.radius, opacity: !canContinue() || isSubmitting ? 0.6 : 1 }]}
        >
          {isSubmitting ? <ActivityIndicator color={colors.primaryForeground} /> : <Text style={[styles.primaryButtonText, { color: colors.primaryForeground }]}>{step === 3 ? "Place Order" : "Continue"}</Text>}
        </Pressable>
      </View>
    </ScrollView>
  );
